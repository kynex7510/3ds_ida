...
if text_base and text_size:
    ...
    if switch_handler != idc.BADADDR:
        plugin_utility.log(PLUGIN_NAME, f"Found switch handler: {hex(switch_handler)}")
        ...
        while switch_addr != idc.BADADDR:
            if switch_addr > text_base and switch_addr < (text_base + text_size):
                plugin_utility.log(PLUGIN_NAME, f"Fixing switch block: {hex(switch_addr)}")
        ...
        plugin_utility.log(PLUGIN_NAME, "Couldn't find switch handler. Nothing to do.")
else:
